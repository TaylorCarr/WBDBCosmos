...
struct CartView: View {
    ...
    @State var fromDate: Date = .init()
    @State var toDate: Date = .init()
    @State var showFromPicker = false
    @State var showToPicker = false
    @State var licensee: String = ""
    @State var dealID: String = .init(Int.random(in: 100_000 ..< 999_999))
        ...
        Group {
            VStack {
                Form {
                    ...
                    DatePicker(selection: self.$fromDate, in: ...Date().addingTimeInterval(1_577_847_600), displayedComponents: .date, label: {
                        Text("FROM")
                    })
                    DatePicker(selection: self.$toDate, in: fromDate ... Date().addingTimeInterval(1_577_847_600), displayedComponents: .date, label: {
                        Text("TO")
                    })
